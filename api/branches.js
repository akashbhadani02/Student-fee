...
async function handler(req,res){res.setHeader('Access-Control-Allow-Origin','*');res.setHeader('Access-Control-Allow-Methods','GET,POST,PUT,DELETE,OPTIONS');res.setHeader('Access-Control-Allow-Headers','Content-Type,Authorization,X-Admin-Password,X-Admin-Action,X-Main-Admin');res.setHeader('Cache-Control','no-store');if(req.method==='OPTIONS')return res.status(204).end();try{await db();if(!(await adminOk(req))||!mainAdmin(req))return res.status(401).json({error:'Main admin access required.'});
 ...
 if(req.method==='PUT'){if(b.type==='branch'){const branch=await Branch.findByIdAndUpdate(b.id,{name:String(b.name||'').trim(),code:String(b.code||'').trim().toUpperCase(),active:b.active!==false},{new:true});if(!branch)return res.status(404).json({error:'Branch not found.'});return res.json({ok:true,branch});}const user=await BranchUser.findById(b.id);if(!user)return res.status(404).json({error:'Branch login not found.'});if(b.username)user.username=String(b.username).trim();if(b.branchId)user.branchId=b.branchId;if(b.password){user.passwordHash=hashPw(String(b.password));user.passwordEncrypted=encPw(String(b.password));}if(b.permissions)user.permissions={...DEFAULT_PERMISSIONS,...b.permissions};await user.save();return res.json({ok:true});}
