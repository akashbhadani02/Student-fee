...
const PASSWORD_KEYS=["pageOpen","studentAdd","studentEdit","studentDelete","collectionAdd","collectionEdit","collectionDelete","commissionPayoutAdd","commissionPayoutEdit","commissionPayoutDelete","excelExport","excelImport"];
...
async function dataRevision(filter){
  const [s,h,p,d]=await Promise.all([
    Student.findOne(filter).sort({updatedAt:-1,_id:-1}).select({updatedAt:1,_id:1}).lean(),
    CollectionHistory.findOne(filter).sort({updatedAt:-1,_id:-1}).select({updatedAt:1,_id:1}).lean(),
    CommissionPayout.findOne(filter).sort({updatedAt:-1,_id:-1}).select({updatedAt:1,_id:1}).lean(),
    DeletedStudent.findOne(filter).sort({updatedAt:-1,_id:-1}).select({updatedAt:1,_id:1}).lean()
  ]);
  return [s,h,p,d].map(x=>x?`${x.updatedAt?.getTime?.()||0}:${x._id}`:'0').join('|');
}
