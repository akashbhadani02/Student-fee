...
// If the database has no students, restore the supplied original data exactly.
if(await Student.countDocuments()===0){const seed=require('./seed-data.json');if(seed.students?.length)await Student.insertMany(seed.students.map(s=>({id:String(s.id),name:String(s.name||'').trim(),mobile:String(s.mobile||'').trim(),branch:String(s.branch||''),branchId:byName.get(String(s.branch||''))?._id||null,totalFee:s.totalFee==null?null:Number(s.totalFee),paidFee:s.paidFee==null?null:Number(s.paidFee)})));}
// Always repair missing/invalid branch IDs without deleting existing records.
...
async function start(){if(!process.env.MONGODB_URI){console.error('MONGODB_URI is missing');process.exit(1);}try{await mongoose.connect(process.env.MONGODB_URI,{serverSelectionTimeoutMS:10000});await ensureBranches();app.listen(PORT,()=>console.log(`Server running on http://localhost:${PORT}`));}catch(e){console.error('MongoDB connection failed:',e.message);process.exit(1);}}
if(require.main===module)start();module.exports=app;
